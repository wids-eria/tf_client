...
public class Interpolate {
    ...
    /// </param>
    /// <returns>
    /// A <see cref="IEnumerable<System.Single>"/>
    /// </returns>
    ...
    /// <summary>
    /// Generic easing sequence generator used to implement the time and slice
    /// variants. Normally you would not use this function directly.
    /// </summary>
    /// <param name="ease">
    /// A <see cref="Function"/>
    /// </param>
    ...
    /// <summary>
    /// Vector3 interpolation using given easing method. Easing is done
    /// independently on all three vector axis.
    /// </summary>
    /// <param name="ease">
    /// A <see cref="Function"/>
    /// </param>
    ...
    /// <summary>
    /// Returns the static method that implements the given easing type for
    /// scalars.
    ...
    /// You can pass zero control points, just the start and end points, for
    /// just plain easing. In other words a zero-degree bezier spline curve is
    /// just the easing method.
    /// </summary>
    /// <param name="ease">
    /// A <see cref="Function"/>
    /// </param>
    /// <param name="points">
    /// A <see cref="Vector3[]"/> start point, n control points, end point
    /// </param>
    /// <param name="elapsedTime">
    /// A <see cref="System.Single"/>
    /// </param>
    ...
    static Vector3 Bezier(Function ease, Vector3[] points, float elapsedTime, float duration) {
        ...
        // Interpolate the n starting points to generate the next j = (n - 1)
		// points, then interpolate those n - 1 points to generate the next
		// n - 2 points, continue this until we have generated the last point
		// (n - (n - 1)), j = 1. We store the next set of output points in the
		// same array as the input points used to generate them. This works
		// because we store the result in the slot of the input point that is
		// no longer used for this iteration.
    ...
    /// <summary>
    /// Returns sequence generator from the first node, through each control
    /// point, and to the last node. N points are generated between each node
    /// (slices) using Catmull-Rom.
    /// </summary>
    /// <param name="nodes">
    /// A <see cref="Transform[]"/>
    /// </param>
    ...
    /// NOTE: The NewCatmullRom() functions are an easier to use alternative to
    /// this raw Catmull-Rom implementation.
    /// </summary>
    /// <param name="previous">
    /// A <see cref="Vector3"/> the point just before the start point or the
    /// start point itself if no previous point is available
    /// </param>
    /// <param name="start">
    /// A <see cref="Vector3"/> generated when elapsedTime == 0
    /// </param>
    /// <param name="end">
    /// A <see cref="Vector3"/> generated when elapsedTime >= duration
    /// </param>
    /// <param name="next">
    /// A <see cref="Vector3"/> the point just after the end point or the end
    /// point itself if no next point is available
    /// </param>
    /// <param name="elapsedTime">
    /// A <see cref="System.Single"/>
    /// </param>
