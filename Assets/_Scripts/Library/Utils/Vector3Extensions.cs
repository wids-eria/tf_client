...
public class Scanner {
	protected static ScannerImpl impl = new ScannerImpl();
	
	public static object[] Scanf( string str, string fieldSpec ) {
		return impl.Scan( str, fieldSpec );
	}
	
	public static void Scanf( string str, string fieldSpec, object[] targets ) {
		impl.Scan( str, fieldSpec, targets );
	}
}

public static class Vector3Extensions {
	public static Vector3 Parse( this Vector3 self, string encodedData ) {
		Vector3 v = new Vector3();
		
		Scanner.Scanf( encodedData, "({Single} {Single} {Single})", new object[] {v.x, v.y, v.z} );
		
		return v;
	}
	
	public static void	Decode( this Vector3 self, object node ) {
		Dictionary<string, object> dict;
		
		ExceptionHelper.Throw<Exception>( JSONDecoder.Decode(node, out dict) );
		
		foreach( KeyValuePair<string, object> kv in dict ) {
			JSONDecoder.SetValue( self, kv.Key, kv.Value );
		}
	}
}